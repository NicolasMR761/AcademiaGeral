const LIST = async () => {
  const res = await window.api.listAlumnos();
  return { ok: res.ok, message: res.message, items: res.alumnos || [] };
};
const CREATE = (a) => window.api.createAlumno(a);
const UPDATE = (id, c) => window.api.updateAlumno(id, c);
const DEL = (id) => window.api.deleteAlumno(id);
let rows = [];
...
async function onTableClick(e) {
  ...
  if (action === "del") {
    ...
    await refresh();
    await afterUI("#nombre"); // o el input principal de alumnos
    return;
  }
}
